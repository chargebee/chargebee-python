...
class ModelTest(unittest.TestCase):
    ...
    def test_sync_list(self, mock_client_class):
        mock_client, mock_response = make_mock_client(text=json.dumps({"list": [customer]}))
        ...
        response = client.Customer.list(client.Customer.ListParams(email=Filters.StringFilter(STARTS_WITH="john")))
    ...
    def test_async_list(self, mock_client_class):
        mock_client, mock_response = make_mock_client(text=json.dumps({"list": [customer]}), is_async=True)
        mock_client.request.return_value = mock_response
        ...
        async def send_request():
            ...
            return await client.Customer.list(client.Customer.ListParams(email=Filters.StringFilter(STARTS_WITH="john")))
